...
export function useGetById<T>(resourceName: string, id: number | string, enabled = true) {
  return useQuery({
...
export function useCreate<TData, TResponse = TData>(resourceName: string) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (data: TData) => apiClient.post<TResponse>(`/${resourceName}`, data),
    onSuccess: () => {
...
export function useUpdate<TData, TResponse = TData>(resourceName: string) {
  const queryClient = useQueryClient();
...
export function useDelete(resourceName: string) {
  ...
  return useMutation({
    mutationFn: (id: number | string) => apiClient.delete(`/${resourceName}/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [resourceName, 'list'] });
    },
  });
